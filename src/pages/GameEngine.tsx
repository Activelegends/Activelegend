...
import { ConnectionProvider, useConnection } from '../game/ConnectionManager';
...
function OnlineGameArea() {
  const { players, myId, status, ping } = useConnection();
  return (
    <div className="w-full max-w-xl mx-auto mt-8">
      <div className="mb-4 flex items-center justify-between px-4 py-2 rounded-xl bg-black/30 text-blue-100 text-sm shadow border border-blue-400/30">
        <span className={`font-bold ${status === 'connected' ? 'text-green-400' : 'text-red-400'}`}>وضعیت: {status === 'connected' ? 'آنلاین' : 'قطع'}</span>
        <span>پلیرها: {players.length}</span>
        <span>پینگ: {ping}ms</span>
      </div>
      <div className="relative w-full h-80 bg-gradient-to-br from-blue-900/40 to-gray-800/60 rounded-2xl border border-white/10 overflow-hidden shadow-lg">
        {players.map((p) => (
          <Player key={p.id} x={p.x} y={p.y} color={p.color} name={p.name} isMe={p.id === myId} />
        ))}
      </div>
    </div>
  );
